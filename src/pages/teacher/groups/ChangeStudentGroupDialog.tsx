  ...
  const loadGroups = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter: faqat o'ziga tegishli guruhlarni ko'rsatish (teacher_id)
        const filtered = groupsList.filter((g: Group) => 
          g.id !== currentGroupId && // Hozirgi guruh emas
          g.teacher?.id === user?.id // O'ziga tegishli
        );
        setGroups(filtered);
      }
    } catch (error) {
            ...
            <div className="space-y-2">
              ...
              {groups.length === 0 ? (
                <p className="text-sm text-muted-foreground py-2">
                  Boshqa guruhlar topilmadi. Avval guruhni tanlang.
                </p>
              ) : (
